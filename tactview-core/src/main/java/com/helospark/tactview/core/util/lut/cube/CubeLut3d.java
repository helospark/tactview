...
public class CubeLut3d extends AbstractCubeLut {
    ...
    public Color apply(Color color) {
        ...
        rgbvec result = interp_tetrahedral(new float[]{blueFloat, greenFloat, redFloat});
     ...
     */
    rgbvec interp_tetrahedral(float[] floatIndices) {
        ...
        rgbvec d = new rgbvec(floatIndices[0] - prev[0], floatIndices[1] - prev[1], floatIndices[2] - prev[2]);
        rgbvec c000 = values[prev[0]][prev[1]][prev[2]];
        rgbvec c111 = values[next[0]][next[1]][next[2]];
        rgbvec c = new rgbvec(0, 0, 0);
        if (d.r > d.g) {
            if (d.g > d.b) {
                rgbvec c100 = values[next[0]][prev[1]][prev[2]];
                rgbvec c110 = values[next[0]][next[1]][prev[2]];
                ...
                rgbvec c100 = values[next[0]][prev[1]][prev[2]];
                rgbvec c101 = values[next[0]][prev[1]][next[2]];
                ...
                rgbvec c001 = values[prev[0]][prev[1]][next[2]];
                rgbvec c101 = values[next[0]][prev[1]][next[2]];
            ...
            if (d.b > d.g) {
                rgbvec c001 = values[prev[0]][prev[1]][next[2]];
                rgbvec c011 = values[prev[0]][next[1]][next[2]];
                ...
                rgbvec c010 = values[prev[0]][next[1]][prev[2]];
                rgbvec c011 = values[prev[0]][next[1]][next[2]];
