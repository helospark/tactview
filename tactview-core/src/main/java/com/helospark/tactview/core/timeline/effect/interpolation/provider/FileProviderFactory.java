...
public class FileProviderFactory extends AbstractKeyframeableEffectDesSerFactory<FileProvider> {
    ...
    @Override
    public FileProvider deserializeInternal(JsonNode data, FileProvider currentFieldValue) {
        FileProvider currentValue = currentFieldValue;
        return new FileProvider(currentValue.getExtension(), ReflectionUtil.deserialize(data.get("stringInterpolator"), StringInterpolator.class, currentFieldValue.stringInterpolator));
    }
